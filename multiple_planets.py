...
class Params:
    ...
    iceline_radius: Optional[u.Quantity] = None
    St_const: Optional[float] = None
    v_frag: u.Quantity = (1 * u.m/u.s).to(u.cm/u.s)
    H_r_model: str = "irradiated"  #4 possible models: Ida_mixed, Liu_mixed, irradiated (flared Ida), flared (Bitsch), flat, Lambrechts_mixed
    dlnP_dlnR: float = -2
...
def evolve_system(
    times: u.s, masses: u.g, positions: u.cm, migration, filtering, peb_acc, params, sim_params):
    ...
    for i in range(sim_params.nr_planets):
        ...
        M_dot[i], R_acc[i], H_peb[i], R_acc_H[i], R_acc_B[i], M_dot_twoD_B[i], M_dot_twoD_H[i],M_dot_threeD_B[i], M_dot_threeD_H[i], M_dot_threeD_unif[i], sigma_peb[i], sigma_gas[i], acc_regimes = peb_acc.dMc_dt_f(times, masses[i], positions[i], F0, flux_reduction, params, sim_params)
        ...
        M_dot[i, dead] = 0  # dM/dt = 0 in case the planet has reached the inner edge or peb iso
        #R_dot[i, dead] = 0  # dR/dt = 0 in case the planet has reached the inner edge or peb iso
    ...
    if np.any(np.isnan(filter_frac)):
        print("Nan in ff")
    return M_dot, R_dot, filter_frac, flux_on_planet, F0, flux_ratio, R_acc, H_peb, R_acc_H, R_acc_B, M_dot_twoD_B, M_dot_twoD_H, M_dot_threeD_B, M_dot_threeD_H,  M_dot_threeD_unif, sigma_peb, sigma_gas, acc_regimes


def simulate_euler(migration, filtering, peb_acc, params, sim_params):
    """Euler solver for the differential equation"""
    args = (migration, filtering, peb_acc, params, sim_params)
    ...
    m_dot, r_dot, filter_f, flux_p, flux , flux_ratio, R_acc, H_peb, R_acc_H, R_acc_B, Mdot_twoD_B, Mdot_twoD_H, Mdot_threeD_B, Mdot_threeD_H, Mdot_threeD_unif, Sigma_peb, Sigma_gas, acc_regimes = evolve_system(t_values[0], mass_values[0], pos_values[0], *args)
    ...
    with open('sims/sim_'+str(sim_params.nr_planets)+'planets_'+str(params.H_r_model)+'_'+str(params.epsilon_el)+'e_el_'+str((params.v_frag).to(u.m/u.s).value)+'.json', 'w') as file:
