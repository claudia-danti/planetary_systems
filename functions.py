...
def omega_k(position: u.cm, params) -> 1 / u.s:
    """ Keplerian frequency """
    return np.sqrt(const.G.cgs * params.star_mass / (position) ** 3)

def v_k(position: u.cm, params) -> u.cm / u.s:
    """ Keplerian velocity """
    return np.sqrt(const.G.cgs * params.star_mass / (position))

def v_hw(position: u.cm, t: u.s, params)-> u.cm/u.s:
    """Headwind velocity experienced by the pebbles"""
    return eta(position, t, params)*v_k(position, params)

def v_H (position: u.cm, mass: u.g, St, params) -> u.cm/u.s:
    """Hill velocity"""
    return 3/2*omega_k(position, params)*b_H(position, mass, St, params)

def R_H(position: u.cm, mass: u.g, params) -> u.cm:
    """ Hill's radius"""
    return (mass/(3*params.star_mass))**(1/3)*position

def R_B(mass: u.g, v: u.cm/u.s)-> u.cm:
    """Bondi radius"""
    return 2*const.G.cgs*mass/(v**2)

def b_H(position: u.cm, mass: u.g, St, params)-> u.cm:
...
def b_B(position: u.cm, mass: u.g, v: u.cm/u.s, St, params)-> u.cm:
    """Impact paramter (aka accretion radius) in Bondi regime"""
    return np.sqrt(v*St/omega_k(position, params)*R_B(mass, v))

def eta(position: u.cm, t: u.s, params):
    """Pressure reduction term"""
    eta_pos_dep = 0.0015*(position/(1*u.au).to(u.cm))**(1/2) #this is assuming a specific H/r
    eta_HR_dep = -1/2*H_R(position, t, params)**2*params.dlnP_dlnR
    return eta_HR_dep

def v_nu (position: u.cm, t: u.s, St, params) -> u.cm/u.s:
...
def v_peb (position: u.cm, t: u.s, St, params)-> u.cm/u.s:
...
def v_r(position: u.cm, t: u.s, St, params) -> u.cm/u.s:
    """Pebble radial drift velocity (see Ida et al. 2016, equation (27))"""
    return v_peb (position, t, St, params) + v_nu(position, t, St, params)

def c_s(T: u.K, params)->u.cm/u.s:
    """Isothermal speed of sound"""
    return np.sqrt(const.k_B.cgs*T/(params.mu*const.m_p.cgs)).to(u.cm/u.s)
    
def T_mid (position: u.cm, t:u.s, params)-> u.K:
    """reverting H/R = c_s / v_K to get T(R)"""
    return (H_R(position, t, params)*v_k(position, params))**2*params.mu*const.m_p.cgs/(const.k_B.cgs.to(u.cm**2*u.g/u.s**2/u.K))

def r_magnetic_cavity(t: u.s, params)-> u.cm:
    """Position of the magnetic cavity according to equation (6) of Liu et al. 2017, assuming a typical T-tauri star with R=2R_sun, M=1M_sun"""
    return (0.1*(M_dot_star(t, params)/(1e-9*const.M_sun.cgs/((1*u.yr).to(u.s))))**(-2/7)*(params.star_magnetic_field/(1e3*u.cm**(-1/2)*u.g**(1/2)/u.s))**(4/7)*u.au).to(u.cm)

def r_magnetic_cavity_gen(t: u.s, params)-> u.cm:
    """Position of the magnetic cavity according to equation (5) of Liu et al. 2017"""
    return (params.star_magnetic_field**4*params.star_radius**12/(4*const.G.cgs*params.star_mass*M_dot_star(t, params)**2))**(1/7)

#### ICELINES #####
# The iceline are computed where T_midplane = 170 K, based on the 
def iceline_irr(t:u.s, T: u.K, params)-> u.cm:
    """Exact calculation using (H/R)_irr = c_s/v_K, with (H/R)_irr from Ida et al.2016"""
    c = 0.024*(params.star_luminosity/const.L_sun.cgs)**(1/7)*(params.star_mass/const.M_sun.cgs)**(-4/7)
    c_s_ice = c_s(T, params) #water iceline is defined where T=170K
    return ((c/c_s_ice*np.sqrt(const.G.cgs*params.star_mass/((1*u.au).to(u.cm))))**(14/3)*u.au).to(u.cm)

def iceline_visc(t:u.s, T: u.K, params)-> u.cm:
    ...
    c_s_ice = c_s(T, params) # water iceline is defined where T=170K
    return ((c/c_s_ice*np.sqrt(const.G.cgs*params.star_mass/((1*u.au).to(u.cm))))**(20/9)*u.au).to(u.cm)

def iceline(t:u.s, T: u.K, params)-> u.cm:
...
def st_d(position: u.cm, t: u.s, params):
...
def st_drift_epstein(position:u.cm, t:u.s, params):
...
def st_drift_stokes(position: u.cm, t: u.s,  params):
...
def st_drift(position: u.cm, t: u.s,  params):
...
def st_frag(position: u.cm, t: u.s, params):
...
def st_frag_drift(position: u.cm, t:u.s, params):
...
def rho_0_t (position:u.cm, t:u.s, params) -> u.g/u.cm**2:
    """Midplane gas density assuming isothermal disc"""
    sigma_gas = sigma_gas_steady_state(position, t, params)
    return sigma_gas/(np.sqrt(2*np.pi)*H_R(position, t, params)*position)

def rho_0 (position:u.cm, t:u.s, sigma_gas: u.g/u.cm**2, params) -> u.g/u.cm**2:
    """Midplane gas density assuming isothermal disc"""
    return sigma_gas/(np.sqrt(2*np.pi)*H_R(position, t, params)*position)

def mfp (position:u.cm, t:u.s, sigma_gas: u.g/u.cm**2, params)-> u.cm:
    """Mean free path of the gas"""
    return (params.mu*const.m_p.cgs)/(params.cross_sec_H*rho_0(position, t, sigma_gas, params))

def st_from_r_peb(r_peb: u.cm, position: u.cm, t: u.s, params):
...
def r_peb_from_st(st: float, position: u.cm, t: u.s, params):
...
def M_dot_star_t_old(t: u.s) -> u.g/u.s:
    """Time dependent gas accretion rate from equation (4) Bitsch et al. 2015 discs, Hartmann et al. 1998"""
    return 10**(-8-1.4*np.log10(t/(1e6 * u.yr).to(u.s)))*const.M_sun.cgs/(1* u.yr).to(u.s)

def M_dot_star_t(t: u.s) -> u.g/u.s:
    #Time dependent gas accretion rate accordin to Hartmann et al. 2016 (ann.rev.), lower limit
    return 10**(((-1.32)-(1.07)*np.log10(t/((1*u.yr).to(u.s)))))*const.M_sun.cgs/(1* u.yr).to(u.s)

def M_dot_star_t_new(t: u.s) -> u.g/u.s:
    return 4*10**(-8)*const.M_sun.cgs/(1*u.yr).to(u.s)*(t/(1*u.Myr).to(u.s))**(-1)
def M_dot_star_t_new_lower(t: u.s) -> u.g/u.s:
    return 4*10**(-8.5)*const.M_sun.cgs/(1*u.yr).to(u.s)*(t/(1*u.Myr).to(u.s))**(-1)


def M_dot_star(t: u.s, params) -> u.g/u.s:
...
def sigma_gas_t(position: u.cm, t: u.s, params) -> u.g / u.cm ** 2:
    """time dependent gas surface density according to LJ14 eq 1"""
    beta = params.beta0 * np.exp(-t / params.tau_disc)
    return beta * (position / (1 * u.au).to(u.cm)) ** (-1)

def sigma_gas_irr(position: u.cm, t: u.s, params) -> u.g/u.cm**2:
    """Sigma gas for an irradiated disc, equation (13) from Ida et al. 2016"""

    return 2.7*1e3*(params.star_mass/(const.M_sun.cgs))**(9/14)*(params.star_luminosity/const.L_sun.cgs)**(-2/7)*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))*(params.alpha/1e-3)**(-1)*(position/(1*u.au).to(u.cm))**(-15/14)*u.g/u.cm**2

def sigma_gas_visc_Ida(position: u.cm, t:u.s, params) -> u.g/u.cm**2:
    """Sigma gas for a viscous disc, equation (12) from Ida et al. 2016"""

    return 2.1*1e3*(params.star_mass/(const.M_sun.cgs))**(1/5)*(params.alpha/1e-3)**(-4/5)*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))**(3/5)*(position/(1*u.au).to(u.cm))**(-3/5)*u.g/u.cm**2

def sigma_gas_visc_Liu(position:u.cm, t:u.s, params) -> u.g/u.cm**2:
    """Sigma gas for viscous disc from Liut et al. 2019, equation (8)"""

    return 132*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))**(1/2)*(params.star_mass/(const.M_sun.cgs))**(1/8)*(params.alpha/1e-2)**(-3/4)*(params.disc_opacity/1e-2)**(-1/4)*(position/(1*u.au).to(u.cm))**(-3/8)*u.g/u.cm**2

def sigma_gas_steady_state(position: u.cm, t:u.s, params) -> u.g/u.cm**2:
...
def sigma_peb(frac_peb, position: u.cm) -> u.g / (u.cm ** 2):
    """pebble surface density as Sigma_peb = f*Sigma_gas"""

    return frac_peb * 1700 * u.g * u.cm ** -2 * (position / (1 * u.au).to(u.cm)) ** -1
...
def sigma_peb_t(position: u.cm, t: u.s, sigma_gas: u.g / u.cm ** 2, params) -> u.g / u.cm ** 2:
...
def sigma_gap_sigma_gas(position: u.cm, mass: u.g, t:u.s, params) -> float:
    """ratio between gap and gas surface density as in Johansen 2019 eq. 37"""
    return 1/(1+(mass/(2.3*M_peb_iso(position, t, params)))**2)

def sigma_gap_sigma_gas_gen(position: u.cm, mass: u.g, t:u.s, params) -> float:
...
def flux_peb_t(t: u.s, position: u.cm, params) -> u.g / u.s:
...
def flux_const(t: u.s, params, sim_params) -> u.g/u.s:
    ...
    pos = (5*u.au).to(u.cm)
    t_in = (1e5*u.yr).to(u.s) # initial time
    t_iso = (2*u.Myr).to(u.s) # we want the planet to reach peb iso at 2 Myrs    
    ...
    if params.H_r_model!='flat':
        ...
        sigma_gas = sigma_gas_t(pos, t_in, params)
    ...
    if params.St_const == None:
        #stokes_peb = st_frag(pos, t, params).value
...
def flux_from_sigma_general(position: u.cm, t: u.s, sigma_peb: u.g / u.cm ** 2, St, params):
...
# This function can be used with a constant Stokes number (St passed as an argument)
def sigma_from_flux_general(position: u.cm, t: u.s, flux: u.g/u.s, St, params) -> u.g/u.cm**2:
...
def flux_from_sigma(position: u.cm, sigma_peb: u.g/u.cm**2, sigma_gas: u.g/u.cm**2, params) -> u.g/u.s: #WARNING NOT TESTED
...
# N.B.: This function intrinsically contains already a NON CONSTANT Stokes number
def sigma_from_flux(position: u.cm, flux: u.g/u.s, sigma_gas: u.g / u.cm ** 2, params) -> u.g/(u.cm **2):
...
########## SCALE HEIGHTS #############
def H_R_irr(position:u.cm, params):
    """Irradiated disc prescription equation (10) in Ida et al. 16"""
    return 0.024*(params.star_luminosity/const.L_sun.cgs)**(1/7)*(params.star_mass/const.M_sun.cgs)**(-4/7)*(position/(1*u.au).to(u.cm))**(2/7)

def H_R_visc_Ida(position:u.cm, t:u.s, params):
    """Viscous disc prescription equation (9) in Ida et al. 16"""

    return 0.027*(params.star_mass/const.M_sun.cgs)**(-7/20)*(params.alpha/1e-3)**(-1/10)*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))**(1/5)*(position/(1*u.au).to(u.cm))**(1/20)

def H_R_visc_Liu(position: u.cm, t:u.s, params):
    """Viscous H/r from equation (10) in Liu et al. 2019"""
    return 0.034*(params.star_mass/const.M_sun.cgs)**(-5/16)*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))**(1/4)*(params.alpha/1e-2)**(-1/8)*(position/(1*u.au).to(u.cm))**(-1/16)*(params.disc_opacity/1e-2)**(1/8)

def H_R_visc_Lambrechts(position: u.cm, t: u.s, params):
    """Viscous H/r from equation (A47) in Lambrechts notes"""
    return 0.019*(params.epsilon_el/1e-2)**(1/10)*(params.epsilon_heat/0.5)**(1/10)*(params.alpha/1e-2)**(-1/10)*(params.Z/0.01)**(1/10)*(params.a_gr/(0.1*u.mm).to(u.cm))**(-1/10)*(params.rho_gr/(1*u.g/u.cm**3))**(-1/10)*(position/(1*u.au).to(u.cm))**(1/20)*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))**(1/5)*(params.star_mass/const.M_sun.cgs)**(-7/20)

def H_R_visc_Lam(position: u.cm, t: u.s, params):
    return 0.04*(params.alpha/1e-3)**(-1/10)*(position/(1*u.au).to(u.cm))**(1/20)*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))**(1/5)

def H_R_visc_Lam_gen(position: u.cm, t: u.s, params):
    return 0.019*(params.epsilon_el/1e-2)**(1/10)*(params.epsilon_heat/0.5)**(1/10)*(params.alpha/1e-2)**(-1/10)*(params.Z/0.01)**(1/10)*(params.a_gr/(0.1*u.mm).to(u.cm))**(-1/10)*(params.rho_gr/(1*u.g/u.cm**3))**(-1/10)*((position/(1*u.au).to(u.cm)))**(1/20)*(M_dot_star(t, params)/(1e-8*const.M_sun.cgs/((1*u.yr).to(u.s))))**(1/5)*(params.star_mass/const.M_sun.cgs)**(-7/20)

def H_R_flared(position:u.cm, params):
    """Flared disc Bitsch et al. 2015"""
    #this is basically the optically thin limit of H/R Ida equations but without M* and L*
    return 0.033 * (position / ((1 * u.au).to(u.cm))) ** (1 / 4)

def H_R(position: u.cm, t: u.s, params):
...
def H_peb(St, position: u.cm, t: u.s, params) -> u.cm:
...
def M_peb_iso(position:u.cm, t:u.s, params) -> u.g:
    """pebble isolation mass according to annual review equations"""
    return 20 * (H_R(position, t, params) / 0.05)**3 *(params.star_mass/const.M_sun.cgs)* ((1 * u.earthMass).to(u.g))

def M_peb_iso_Bitsch(position: u.cm, t: u.s, params) -> u.g:
    ...
    l = 0.00476/f_fit
    return 25*f_fit*((1 * u.earthMass).to(u.g)) + Pi_crit/l*((1 * u.earthMass).to(u.g))

def M_Bondi_Hill_trans(position: u.cm, t: u.s, params) -> u.g:
    ...
    if params.St_const == None:
        ...
        stokes_peb = params.St_const
    return v_hw(position, t, params)**3/(8*const.G.cgs*omega_k(position, params)*stokes_peb)*(4/3)**2

def M_Bondi_Hill_trans_debug(position: u.cm, t: u.s, stokes, params) -> u.g:
    return (4/3*omega_k(position, params))**2*(eta(position, t, params)*position/2)**3/(stokes*const.G.cgs)

def M_cut(position: u.cm, t:u.s, params) -> u.g:
    ...
    if params.St_const == None:
        ...
        stokes_peb = params.St_const
    return 1e-6*stokes_peb/1e-2*(eta(position, t, params)/1.8e-3)**3*(1*u.earthMass).to(u.g)

def M_peb_cutoff(position: u.cm, t:u.s, params) -> u.g:
    """Lower limit for pebble accretion, according to equation (17) in the notes"""
    if params.St_const == None:
        stokes_peb = st_frag_drift(position, t, params)
    else: 
        stokes_peb = params.St_const
    return 2.5*1e-4*(stokes_peb/0.1)*(eta(position, t, params)/0.002)**3*(params.star_mass/const.M_sun.cgs)*(1*u.earthMass).to(u.g)


# WARNING I used alpha_z so it could not work when changing alpha across the iceline
def M_3D_2DH_trans(position: u.cm, t: u.s, params) -> u.g:
    if params.St_const == None:
        #stokes_peb = st_frag(position, t, params)
        stokes_peb = st_frag_drift(position, t, params)
    ...
    return 3/4*(2*H_peb(stokes_peb, position, t, params)*(np.sqrt(2*np.pi))/np.pi)**3*(omega_k(position, params)**2/(const.G.cgs*stokes_peb))

def M_3D_2DB_trans (position: u.cm, t: u.s, params) -> u.g:
    if params.St_const == None:
        #stokes_peb = st_frag(position, t, params)
        stokes_peb = st_frag_drift(position, t, params)
    ...
    return (8*H_peb(stokes_peb, position, t, params)**2/np.pi)*eta(position, t, params)*omega_k(position, params)**2*position/(const.G.cgs*stokes_peb)

def M0_pla(position: u.cm, t: u.s, sigma_gas: u.g/u.cm**2, params)-> u.g:
    """Initial planetesimal mass scaling, according to Liu et al. 2020"""
    f = 400
    f_value = f/400
    return (2e-4*f_value*(H_R(position, t, params)/0.04)**(3/2)*(sigma_gas/(1700*u.g/u.cm**2))**(3/2)*(position/(1 * u.au).to(u.cm))**3*u.M_earth).to(u.g)

def pebble_prod_line(time: u.s, params) -> u.cm:
    """pebble production line according to eq 10 of LJ14"""

    return (3 / 16) ** (1 / 3) * (const.G.cgs * params.star_mass) ** (1 / 3) * (params.epsilon_d * params.Z) ** (2 / 3) * time ** (2 / 3)

def t_acc(mass: u.g, F: u.g/u.s, filter_frac)-> u.s:
